...
### Дообновление: частота и гигиена issue-трекера

После первых нескольких прогонов (ручных, в ходе отладки скрипта и фиксов A–E) в трекере накопилось пять открытых `Nightly Lighthouse Report` issue
подряд — обнаружилось, что старые не закрываются автоматически. Внесены две правки:

- **Расписание сокращено** с ежедневного (`0 22 * * *`) до еженедельного (`0 22 * * 0`, воскресенье) — ежедневный прогон избыточен для темпа
  изменений в учебном проекте, только создавал шум.
- **Автозакрытие предыдущего отчёта** — новый шаг `Close previous nightly reports` перед созданием issue: находит все открытые issue с заголовком
  `Nightly Lighthouse Report` через `gh issue list --search` и закрывает их с комментарием `Superseded by the next nightly Lighthouse run`. Теперь в
  трекере в любой момент не больше одного открытого отчёта.

Старые пять issue (#12, #22, #26, #27, #31) закрыты вручную одноразово, дальше процесс полностью автоматический.
...
## 12. Финальный аудит интеграции (шаг завершён, смёржен в main)

**Цель шага:** не новые фичи, а проверка, что весь agentic-процесс в GitHub устойчив, безопасен и предсказуем — триггеры, permissions, контроль расходов.

### Найденные и устранённые пробелы

1. **Отсутствовала защита от самозапуска/ботов** в `opencode.yml` — `if:` проверял только текст комментария (`/oc`), не автора. Реальный риск:
   агент цитирует предыдущие комментарии при построении контекста, и если бы цитата содержала `/oc`, это могло бы запустить агента на самого себя.
   Фикс: `github.event.comment.user.type != 'Bot'` в условии job.
2. **Три workflow без явного `permissions:`** (`commitlint.yml`, `e2e.yml`, `pr-title.yml`) — наследовали repo-wide дефолт вместо явно
   заданных минимальных прав. Практического риска не было (все три только читают код), но не соответствовало принципу "явно заданный минимум,
   видимый в диффе". Добавлены явные блоки (`contents: read`, у `pr-title.yml` дополнительно `pull-requests: read`).

### Контроль расходов — уже приведено в порядок на предыдущих шагах

- `lighthouse.yml`: расписание сокращено с ежедневного до еженедельного (воскресенье 22:00 UTC) — соответствует реальному темпу изменений в
  проекте.
- Модель одна и та же (`deepseek/deepseek-v4-pro`) для всех `/oc`-задач — осознанный выбор, не забытая настройка: DeepSeek уже бюджетный провайдер,
  дифференциация по сложности задачи признана избыточной для масштаба проекта.

### Подтверждённые сценарии

Все три обязательных сценария отработаны многократно за время проекта: вызов агента в issue (`/oc explain`), агент создаёт PR и проходит ревью
(PR #5, #7, #14, #16, #24, #25 и другие), scheduled workflow запускался и по расписанию, и вручную (`Nightly Lighthouse Check`).

### Самооценка эффективности агента

**С первого раза, без доработок по содержанию:** триаж `/oc explain` по пустым слотам (точный разбор без наводящих вопросов), Issue C (контраст в
шапке), содержательная часть Issue A/B (`robots.txt`, meta description).

**Потребовались содержательные доработки, не только формат:**
`durationMinutes` (Issue 1 — первая попытка добавила валидацию только на фронтенде, не на бэкенде), Issue D→E (фикс контраста не сработал с первого
раза из-за приоритета CSS-переменных в каскаде Mantine).

**Системная проблема, а не разрозненные сбои:** Conventional Commits формат в PR title/commit message — почти каждый `/oc fix`/ответ на review требовал
ручного `git commit --amend`. Явное правило в `AGENTS.md` и напоминания в каждом `/oc`-комментарии снижали частоту, но не устраняли полностью —
принято как известное ограничение модели, задокументировано в разделе 9.

**Вывод:** содержательных провалов было меньше, чем процессных (формат коммитов) — и последние однотипны и предсказуемы, а не хаотичны. Хороший результат по критерию шага.